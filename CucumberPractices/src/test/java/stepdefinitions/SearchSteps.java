...
public class SearchSteps {
	...
	@When("^I search for a product with name \"([^\"]+)\" and price (\\d+)$") //This step alone, i used regular Expression, MANDATORY FOLLOW LIKE @Then("^sample steps$") FOR REGULAR EXPRESSION and remaining steps are having cucumber expressions
	public void i_search_for_a_product_with_name_and_price(String productName, Integer price) {
	    System.out.println("Step 2: Search the product with name: " + productName + " price: " + price);
	    product = new Product(productName, price);
	}

	@Then("Product with name {string} should be displayed") //mostly @Then used for validation, so we used assertions
	...
	public void order_id_is_and_username_is(Integer orderId, String userName) {
	    System.out.println("Step 4: order id is : "+ orderId +" username is : " + userName);
	}
}